...
class BidirectionalRRTPlannerTest(unittest.TestCase):
    ...
    def test_connect1(self):
        ...
        Description
        -----------
        This function tests the connect function of the BidirectionalRRTPlanner.
        ...
        # Use connect to try to move towards the position goal_config
        planner.root_context = self.diagram.CreateDefaultContext()
        nearest_node_to_goal, dist = planner.sample_nearest_in_tree(rrt0, q_goal)
        q_new, reached_goal = planner.connect(
            nearest_node_to_goal,
            q_goal,
            rrt0,
            tree_is_goal=False,
        )
