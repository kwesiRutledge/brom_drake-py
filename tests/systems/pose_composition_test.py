    ...
    def test_correctness1(self):
        ...
        *Description*
        ...
        pose2 = RigidTransform(
            p=np.array([-2.0, 0.3, 0.1]), rpy=RollPitchYaw(np.pi, 0.0, 0.0)
        )
        ...
        composition_system = builder.AddNamedSystem(
            system=PoseCompositionSystem(), name="composer"
        )

        builder.Connect(
            source1.get_output_port(), composition_system.GetInputPort("pose_AB")
        )
        builder.Connect(
            source2.get_output_port(), composition_system.GetInputPort("pose_BC")
        )
        ...
        output_pose_port_watcher: PortWatcher = composition_system_port_watchers[
            "pose_AC"
        ]
