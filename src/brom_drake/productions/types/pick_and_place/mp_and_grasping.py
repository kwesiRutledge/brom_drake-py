...
class MotionPlanningAndGraspingProduction(BaseProduction):
    def __init__(
        ...
        **kwargs,
        ...
        self.start_frame, self.goal_frame = None, None
        self.spheres = []
    ...
    def add_start_and_goal_to_plant(self, plant: MultibodyPlant):
        ...
        self.start_frame = FixedOffsetFrame(
            "start_frame",
            plant.world_frame(),
            self.start_pose,
        )
        self.plant.AddFrame(self.start_frame)
        ...
        self.plant.AddFrame(self.goal_frame)
    ...
    def create_highlighted_sphere_region(
        self,
        frame_in: FixedOffsetFrame,
        color: np.ndarray = None,
    ):
        """
        Description
        -----------
        This method creates a sphere that highlights the region
        around the frame `frame`.
        """
        # Setup
        ...
        sphere_shape_defn = SimpleShapeURDFDefinition(
            name=f"sphere{sphere_index}",
            shape=SphereDefinition(radius=0.1),
            create_collision=False,
            color=color,
        )
        self.spheres = self.spheres + [sphere_shape_defn]

        sphere_urdf_location = Path(
            f"brom/models/spheres/{sphere_shape_defn.name}.urdf"
        )
        os.makedirs(sphere_urdf_location.parent, exist_ok=True)
