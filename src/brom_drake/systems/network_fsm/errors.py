...
class EdgeContainsNoLabelsError(ValueError):
    ...
    def __init__(self, edge: Tuple[int, int]):
        self.edge = edge
        self.message = (
            f"Edge {edge} contains no labels (i.e., it is an empty dictionary);\n"
        )
        self.message += (
            f'Need at least one label (the label should be called "conditions")!\n'
        )
...
class OutputPortNotInitializedError(ValueError):
    ...
    def __init__(
        self, node_index: int, output_port: str, output_ports_initialized: List[str]
    ):
        self.output_port = output_port
        self.message = f'Output port "{output_port}" for node #{node_index} does not have an initial value in the FSM graph;\n'
        self.message += (
            f"The ports that were initialized are {output_ports_initialized}!\n"
        )
        super().__init__(self.message)
