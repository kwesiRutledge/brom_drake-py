...
class PortIsNotFoundInDiagramError(ValueError):
    ...
    **Description**
    ...
    def __init__(self, target: DiagramTarget, port_reference: int|str, port_names: List[str] = None):
        self.target = target
        self.port_reference = port_reference
        self.message = (
            f"Port \"{port_reference}\" on system with name {target.name} was not found in the Diagram.\n"
        )
...
class PortIsNotBeingWatchedError(ValueError):
    ...
    **Description**
    ...
    def __init__(self, target: DiagramTarget, port_reference: int|str,  port_names: List[str] = None):
        self.target = target
        self.message = (
            f"Port \"{port_reference}\" on system with name {target.name} is not being watched by the DiagramWatcher.\n"
        )
        if port_names is not None:
            self.message += f"Available ports are:\n"
            for port_index, port_name in enumerate(port_names):
...
class SystemIsNotBeingWatchedError(ValueError):
    ...
    **Description**
    ...
    def __init__(self, target: DiagramTarget, system_names: List[str] = None):
        self.target = target
        self.message = (
            f"System with name \"{target.name}\" is not being watched by the DiagramWatcher.\n"
        )
        if system_names is not None:
            self.message += f"Available systems are:\n"
            for system_index, system_name in enumerate(system_names):
