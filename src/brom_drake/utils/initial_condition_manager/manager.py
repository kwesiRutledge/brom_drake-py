...
class InitialConditionManager:
    ...
    def add_initial_configuration(
        self, model_instance_index: ModelInstanceIndex, configuration: np.ndarray
    ):
        ic_tuple = InitialCondition(
            model_instance_index=model_instance_index, configuration=configuration
        )
        self._ic_tuples.append(ic_tuple)

    def add_initial_pose(
        self, model_instance_index: ModelInstanceIndex, pose_wrt_parent: RigidTransform
    ):
        ic_tuple = InitialCondition(
            model_instance_index=model_instance_index, pose_wrt_parent=pose_wrt_parent
        )
        self._ic_tuples.append(ic_tuple)

    def set_all_initial_conditions(
    ...
    def set_all_initial_configurations(
        self, plant: MultibodyPlant, diagram_context: Context = None
    ):
